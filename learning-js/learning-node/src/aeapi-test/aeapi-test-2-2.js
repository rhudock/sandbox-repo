...
request(options, function (error, response, body) {
    ...
    {
        eaapiLogin(response, body)
    }

});


function eaapiLogin(response, body){
    ...
    request(optionsLogin, function (error, response, body) {
        ...
        if (!error && response.statusCode == 200)
        {
            ...
            aeapiGetEngagement(response, body)
        } else {
...
function aeapiGetEngagement(response, body) {
    // Print out the response body
    ...
    request(optionsGet, function (error, response, body) {
        ...
        if (!error && response.statusCode == 200) {
            if( JSON.parse(body).engagements ) {
                aeapiEngageChat(response, body);
            } else {
                console.log(body);
            }
        } else {
...
function aeapiEngageChat(response, body) {
    // Print out the response body
    ...
    request(optionsEngage, function (error, response, body) {
        ...
        if (response.statusCode == 200) {
            aeapiPostMessage(response, body)
        } else {
            console.log(response.statusCode);
            console.log(response.headers);
            console.log(body);
...
function aeapiPostMessage(response, body) {
    console.log(`> Engage Chat: ${engagementId} successful`);
    console.log(body);
    // console.log(response.headers);

    var jsonMsg = `{ 'creditcare': ['1234123412341234', '2345234523452345'] , 'name':'Tester me' }`;
    ...
    request(optionsPost, function (error, response, body) {
        ...
        if (response.statusCode == 200) {
            ...
            // Print out the response body
            console.log(`{  \"agentId\": \"a1\",  \"chatLineReceiverType\": \"external\",  \"engagementId\": \"${engagementId}\",  \"messageText\": \"${jsonMsg}\",  \"messageType\": \"dynamicForm\"}`);
            // console.log(response.headers);
