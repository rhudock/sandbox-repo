package com.inq.flash.agent.view {
	...
	public class CheckTreeRenderer extends TreeItemRenderer	{
		...
		private static var STATE_SCHRODINGER:String = "schrodinger";
		private static var STATE_CHECKED:String = "checked";
		private static var STATE_UNCHECKED:String = "unchecked";
		
		public static const CHECK_BOX_CLICKED_EVENT:String = "CheckBoxChanged";
		...
		private function getState(tree:Tree, parent:Object):String{
			...
			if (parent != null)	{
				...
				while (!cursor.afterLast){
					...
					else if (!cursor.current.hasOwnProperty("@state") || cursor.current.@state == STATE_UNCHECKED){
						noUnChecks++
					}
					else {
						noCats++;
		...
		private function checkBoxToggleHandler(event:MouseEvent):void {
			...
			if (data) {
				...
				event.target.dispatchEvent(new Event("CheckBoxChanged", true, false));
				checkBoxClicked = true;
			}
		}
		
		private function labelChanged(event:MouseEvent):void {
			if (!checkBoxClicked) {
		...
		override protected function createChildren():void {
			...
			addChild(myCheckBox);
			this.addEventListener(MouseEvent.CLICK, labelChanged);
		...
		private function setCheckState (checkBox:CheckBox, value:Object, state:String):void	{
			...
			else if (state == STATE_UNCHECKED || state == null || state.length == 0){
